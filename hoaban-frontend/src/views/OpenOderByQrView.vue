...
// Ordered items count
const orderedItemsCount = computed(() => {
...
// Total items (ordered + cart)
const totalItemsCount = computed(() => {
  return orderedItemsCount.value + cartItemsCount.value;
});
          ...
          <div class="flex items-center justify-between">
            ...
            <div class="text-right">
              <div class="text-sm text-white/80">
                <span v-if="orderedItemsCount > 0">Đã đặt: {{ orderedItemsCount }}</span>
                <span v-if="orderedItemsCount > 0 && cartItemsCount > 0"> • </span>
                <span v-if="cartItemsCount > 0">Giỏ: {{ cartItemsCount }}</span>
                <span v-if="orderedItemsCount === 0 && cartItemsCount === 0">Giỏ hàng</span>
              </div>
              <div class="text-2xl font-bold text-white">{{ totalItemsCount }} món</div>
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="lg:col-span-1">
            <div class="sticky top-24 space-y-4">
              ...
              <div
                ...
                class="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl p-4"
              >
                <h3 class="text-lg font-bold text-white mb-4">✅ Món đã đặt</h3>
                <div class="space-y-2 max-h-64 overflow-y-auto">
                  <div
                    v-for="(item, idx) in order.items"
                    :key="idx"
                    class="bg-emerald-500/10 border border-emerald-400/30 rounded-lg p-3"
                  >
                ...
                <div class="mt-3 pt-3 border-t border-white/20">
                  <div class="flex justify-between text-white font-semibold">
                    ...
                    <span class="text-emerald-400">{{ formatPrice(order.subtotal || 0) }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl p-4">
                <h3 class="text-lg font-bold text-white mb-4">🛒 Giỏ hàng mới</h3>
                ...
                <div v-if="cart.length > 0">
                  <div class="border-t border-white/20 pt-4 mb-4">
                    <div class="flex justify-between items-center text-lg font-bold">
                      <span class="text-white">Tổng giỏ mới:</span>
                      <span class="text-emerald-400">{{ formatPrice(cartTotal) }}</span>
                    </div>
                  </div>
